...
export function getPasswordChangedNotificationTemplate(name: string): string {
  const currentYear = new Date().getFullYear();
  return `
<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Sua senha foi alterada — OdontoSync</title>
  <style>
    body {
      margin: 0;
      padding: 0;
...
<body>
  <div class="wrapper">
    <div class="container">
      ...
      <div class="content">
        <h2>Olá, ${name}!</h2>
        <p>Confirmamos que a senha da sua conta no aplicativo <strong>OdontoSync</strong> foi alterada com sucesso recentemente.</p>
        
        <div class="security-notice">
          <div class="security-title">⚠️ Aviso Importante de Segurança</div>
          <p class="security-desc">Se você realizou essa alteração, nenhuma ação adicional é necessária. Caso <strong>não</strong> tenha alterado sua senha, entre em contato imediatamente com a administração da clínica ou utilize a tela de login do aplicativo para redefini-la usando seu e-mail.</p>
        </div>
        
        <p>Sua segurança é nossa prioridade absoluta.</p>
      </div>
      <div class="footer">
        <p>Este é um e-mail de segurança automático enviado pelo sistema de proteção da clínica OdontoSync.</p>
        <p>&copy; ${currentYear} OdontoSync. Todos os direitos reservados.</p>
      </div>
    </div>
  </div>
